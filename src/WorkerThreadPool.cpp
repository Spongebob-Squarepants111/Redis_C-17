...
// WorkerThreadPool实现
WorkerThreadPool::WorkerThreadPool(size_t worker_count, std::shared_ptr<CommandHandler> handler)
    : handler_(handler) {
...
void WorkerThreadPool::assign_client(int client_fd) {
...
void WorkerThreadPool::remove_client(int client_fd) {
    std::lock_guard<std::mutex> lock(mapping_mutex_);
...
WorkerThreadPool::Stats WorkerThreadPool::get_stats() const {
    Stats stats;
