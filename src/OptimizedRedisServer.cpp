...
OptimizedRedisServer::OptimizedRedisServer(const Config& config)
    ...
    worker_pool_ = std::make_unique<WorkerThreadPool>(config.worker_threads, handler_);
    
    start_time_ = std::chrono::steady_clock::now();
}

OptimizedRedisServer::~OptimizedRedisServer() {
    stop();
}

void OptimizedRedisServer::run() {
    try {
        setup_server_socket();
        ...
        // 启动accept线程
        accept_thread_ = std::thread(&OptimizedRedisServer::accept_loop, this);
        std::cout << "Accept thread started" << std::endl;
        
        // 启动统计线程
        stats_thread_ = std::thread(&OptimizedRedisServer::print_stats_loop, this);
...
void OptimizedRedisServer::stop() {
    running_ = false;
...
void OptimizedRedisServer::setup_server_socket() {
...
void OptimizedRedisServer::optimize_socket(int sockfd) {
    int opt = 1;
...
void OptimizedRedisServer::accept_loop() {
...
void OptimizedRedisServer::print_stats_loop() {
    while (running_) {
...
OptimizedRedisServer::Stats OptimizedRedisServer::get_stats() const {
